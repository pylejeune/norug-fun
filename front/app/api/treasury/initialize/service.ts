...
export async function initializeTreasury(): Promise<TreasuryInitializeResult> {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Erreur lors de la récupération du compte treasury:",
        fetchError
      );
    }
    ...
    console.log(
      "Compte Treasury sur la blockchain:",
      accountInfo ? "Existe" : "N'existe pas"
    );
          ...
          accounts: {
            marketing: {
              ...
              lastWithdrawal: new Date(
                treasuryData.marketing.last_withdrawal * 1000
              ).toString(),
            ...
            team: {
              ...
              lastWithdrawal: new Date(
                treasuryData.team.last_withdrawal * 1000
              ).toString(),
              publicKey: treasuryPDA.toString(),
            },
            operations: {
              ...
              lastWithdrawal: new Date(
                treasuryData.operations.last_withdrawal * 1000
              ).toString(),
              publicKey: treasuryPDA.toString(),
            },
            investments: {
              ...
              lastWithdrawal: new Date(
                treasuryData.investments.last_withdrawal * 1000
              ).toString(),
              publicKey: treasuryPDA.toString(),
            },
            crank: {
              ...
              lastWithdrawal: new Date(
                treasuryData.crank.last_withdrawal * 1000
              ).toString(),
              publicKey: treasuryPDA.toString(),
            },
          },
    ...
    const tx = (program as any).methods
      .initializeTreasury(adminKeypair.publicKey)
      .accounts({
    ...
    try {
      if ((program.account as any).treasury) {
        newTreasuryAccount = await (program.account as any).treasury.fetch(
          treasuryPDA
        );
      }
    } catch (error) {
      console.error(
        "Erreur lors de la récupération du compte treasury après initialisation:",
        error
      );
    }
    ...
    if (newTreasuryAccount) {
      ...
      return {
        success: true,
        message:
          "Treasury initialisée avec succès, mais les détails ne sont pas disponibles.",
      };
    }
  } catch (error: unknown) {
    console.error(
      "Erreur lors de la vérification/initialisation de la treasury:",
      error
    );
