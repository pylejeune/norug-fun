...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  console.log(`[${requestId}] 🚀 Création d'une époque de 24 heures...`);
  ...
  try {
    const result = await createEpoch(24 * 60); // 24 heures en minutes
    console.log(`[${requestId}] ✅ Opération terminée: ${result.message}`);

    return createSuccessResponse(requestId, result);
  } catch (error) {
    console.error(
      `[${requestId}] ❌ Erreur lors de la création de l'époque:`,
      error
    );
