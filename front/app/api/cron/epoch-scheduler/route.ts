...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    console.log(
      `[${requestId}] 📊 Nombre total d'époques: ${allEpochs.length}`
    );
    ...
    console.log(
      `[${requestId}] 📊 Nombre d'époques actives: ${activeEpochs.length}`
    );
    ...
    console.log(
      `[${requestId}] 📊 Nombre d'époques expirées: ${expiredEpochs.length}`
    );
    ...
    if (expiredEpochs.length > 0) {
      ...
      if (!result.activeEpochs || result.activeEpochs.length === 0) {
        console.log(
          `[${requestId}] 🔄 Aucune époque active, création d'une nouvelle époque...`
        );
        ...
        if (newEpochResult.success) {
          console.log(
            `[${requestId}] ✅ Nouvelle époque créée avec succès: ${newEpochResult.epoch?.id}`
          );
          ...
          console.error(
            `[${requestId}] ❌ Erreur lors de la création d'une nouvelle époque:`,
            newEpochResult.error
          );
        }
      } else {
        console.log(
          `[${requestId}] ℹ️ Il reste ${result.activeEpochs.length} époque(s) active(s), pas besoin d'en créer une nouvelle.`
        );
      }
    } else {
      ...
      if (activeEpochs.length === 0) {
        ...
        if (newEpochResult.success) {
          ...
          result = {
            success: true,
            message:
              "Aucune époque expirée, mais une nouvelle époque a été créée",
        ...
        console.log(
          `[${requestId}] ✅ Aucune époque expirée, et ${activeEpochs.length} époque(s) active(s).`
        );
    ...
    console.log(
      `[${requestId}] ✅ Vérification des époques terminée. Époques expirées fermées: ${expiredEpochs.length}, Nouvelle époque créée: ${newEpochCreated}`
    );

    return createSuccessResponse(requestId, result);
  } catch (error) {
    console.error(
      `[${requestId}] ❌ Erreur lors de la vérification des époques:`,
      error
    );
