...
export async function initializeProgramConfig(): Promise<ProgramConfigResult> {
  ...
  console.log(
    `🔑 PDA du ProgramConfig calculé: ${programConfigPDA.toString()}`
  );
  ...
  try {
    ...
    try {
      if (program.account && typeof program.account === "object") {
        ...
        if ((program.account as any).programConfig) {
          programConfigAccount = await (
            program.account as any
          ).programConfig.fetchNullable(programConfigPDA);
          console.log(
            "ProgramConfig trouvé:",
            programConfigAccount ? "Oui" : "Non"
          );
        } else {
          console.log(
            "Le compte programConfig n'existe pas dans program.account"
          );
        }
      }
    } catch (fetchError) {
      console.error(
        "Erreur lors de la récupération du compte ProgramConfig:",
        fetchError
      );
    }
    ...
    console.log(
      "Compte ProgramConfig sur la blockchain:",
    ...
    try {
      if ((program.account as any).programConfig) {
        newProgramConfigAccount = await (
          program.account as any
        ).programConfig.fetch(programConfigPDA);
        console.log("✅ ProgramConfig initialisé et récupéré avec succès");
      }
    } catch (error) {
      console.error(
        "Erreur lors de la récupération du ProgramConfig après initialisation:",
        error
      );
    }
    ...
    if (newProgramConfigAccount) {
      ...
      return {
        success: true,
        message:
          "ProgramConfig initialisé avec succès, mais les détails ne sont pas disponibles.",
      };
    }
  } catch (error: unknown) {
    console.error(
      "Erreur lors de la vérification/initialisation du ProgramConfig:",
      error
    );
