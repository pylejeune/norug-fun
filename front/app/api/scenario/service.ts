...
export async function createScenario() {
  ...
  try {
    ...
    if (!authToken) {
      throw new Error("API_SECRET_KEY non défini dans les variables d'environnement");
    }
    ...
    for (let i = 0; i < 10; i++) {
      ...
      console.log(`[${requestId}] 📤 Envoi de la requête de création d'époque...`);
      const epochResponse = await fetch('http://localhost:3000/api/epoch/create', {
        method: 'GET',
        headers: {
      ...
      if (!epochResponse.ok) {
        throw new Error(`Erreur lors de la création de l'époque ${i + 1}: ${epochResponse.statusText}`);
      }
      ...
      console.log(`[${requestId}] 📝 Création des propositions pour l'époque ${i + 1}...`);
      for (let j = 0; j < 15; j++) {
        console.log(`[${requestId}] 📤 Création de la proposition ${j + 1}/15...`);
        
        // Appel à l'endpoint de création de proposition
        const proposalResponse = await fetch('http://localhost:3000/api/proposal/create', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`,
        ...
        if (!proposalResponse.ok) {
          throw new Error(`Erreur lors de la création de la proposition ${j + 1} pour l'époque ${i + 1}: ${proposalResponse.statusText}`);
        }

        const proposalResult = await proposalResponse.json();
        console.log(`[${requestId}] 📥 Proposition ${j + 1} créée avec succès:`, {
    ...
    console.error(`[${requestId}] ❌ Erreur lors de la création du scénario:`, error);
    throw error;
  }
} 